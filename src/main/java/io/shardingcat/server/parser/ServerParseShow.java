...
package io.shardingcat.server.parser;
...
public final class ServerParseShow {
	...
	public static final int SHARDINGCAT_STATUS = 3;
	public static final int SHARDINGCAT_CLUSTER = 4;
	...
	public static int parse(String stmt, int offset) {
		...
		for (; i < stmt.length(); i++) {
			...
			case 'M':
			case 'm':
				return shardingcatCheck(stmt, i);
			case 'D':
			case 'd':
	...
	// SHOW SHARDINGCAT_
	static int shardingcatCheck(String stmt, int offset) {
		if (stmt.length() > offset + "ycat_?".length()) {
			...
			if ((c1 == 'Y' || c1 == 'y') && (c2 == 'C' || c2 == 'c')
				...
				case 'S':
				case 's':
					return showShardingCatStatus(stmt, offset);
				case 'C':
				case 'c':
					return showShardingCatCluster(stmt, offset);
				default:
					return OTHER;
				}
			}
	...
	// SHOW ShardingCat_STATUS
	static int showShardingCatStatus(String stmt, int offset) {
		if (stmt.length() > offset + "tatus".length()) {
			...
			if ((c1 == 't' || c1 == 'T')
					...
					&& (stmt.length() == ++offset || ParseUtil.isEOF(stmt
							.charAt(offset)))) {
				return SHARDINGCAT_STATUS;
			}
		}
		return OTHER;
	}

	// SHOW ShardingCat_CLUSTER
	static int showShardingCatCluster(String stmt, int offset) {
