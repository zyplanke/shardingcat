...
import io.shardingcat.server.handler.MysqlInformationSchemaHandler;
import io.shardingcat.server.handler.MysqlProcHandler;
import io.shardingcat.server.parser.ServerParse;
import io.shardingcat.server.response.Heartbeat;
import io.shardingcat.server.response.InformationSchemaProfiling;
import io.shardingcat.server.response.Ping;
...
public class ServerConnection extends FrontendConnection {
	...
	public void execute(String sql, int type) {
		...
		if (db == null) {
			...
			if (db == null) {
				writeErrMessage(ErrorCode.ERR_BAD_LOGICDB, "No ShardingCat Database selected");
				return;
			}
			isDefault = false;
		}
		...
		SchemaConfig schema = ShardingCatServer.getInstance().getConfig().getSchemas().get(db);
		if (schema == null) {
			writeErrMessage(ErrorCode.ERR_BAD_LOGICDB,
					"Unknown ShardingCat Database '" + db + "'");
			return;
		}
		...
		if (isDefault && schema.isCheckSQLSchema() && isNormalSql(type)) {
			...
			if (schemaInfo != null && schemaInfo.schema != null && !schemaInfo.schema.equals(db)) {
				SchemaConfig schemaConfig = ShardingCatServer.getInstance().getConfig().getSchemas().get(schemaInfo.schema);
    ...
    public RouteResultset routeSQL(String sql, int type) {
		...
		if (db == null) {
			writeErrMessage(ErrorCode.ERR_BAD_LOGICDB,
					"No ShardingCat Database selected");
			return null;
		}
