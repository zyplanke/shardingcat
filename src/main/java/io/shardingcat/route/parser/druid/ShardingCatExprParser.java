 ...
 */
public class ShardingCatExprParser extends MySqlExprParser
{
    public static final String[] max_agg_functions = {"AVG", "COUNT", "GROUP_CONCAT", "MAX", "MIN", "STDDEV", "SUM", "ROW_NUMBER"};

    public ShardingCatExprParser(Lexer lexer)
    {
        super(lexer);
        super.aggregateFunctions = max_agg_functions;
    }

    public ShardingCatExprParser(String sql)
    {
        super(new ShardingCatLexer(sql));
        lexer.nextToken();
        super.aggregateFunctions = max_agg_functions;
    }
    @Override
