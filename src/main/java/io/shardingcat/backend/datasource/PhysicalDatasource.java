...
import io.shardingcat.backend.ConMap;
import io.shardingcat.backend.ConQueue;
import io.shardingcat.backend.heartbeat.DBHeartbeat;
import io.shardingcat.backend.mysql.nio.MySQLConnection;
import io.shardingcat.backend.mysql.nio.handler.ConnectionHeartBeatHandler;
import io.shardingcat.backend.mysql.nio.handler.DelegateResponseHandler;
import io.shardingcat.backend.mysql.nio.handler.NewConnectionRespHandler;
...
public abstract class PhysicalDatasource {
	 ...
	 *   @see https://github.com/ShardingCatApache/ShardingCat-Server/issues/1524
	 *   
	 */
	// 当前活动连接
	...
	/**
	 * 由于在ShardingCat中，returnCon被多次调用（与takeCon并没有成对调用）导致activeCount、totalConnection容易出现负数
	 */
...
//		}
	...
	private void createNewConnection(final ResponseHandler handler,
		...
		ShardingCatServer.getInstance().getBusinessExecutor().execute(new Runnable() {
			public void run() {
				try {
