...
import io.shardingcat.backend.postgresql.PostgreSQLBackendConnection.BackendConnectionState;
import io.shardingcat.backend.postgresql.packet.AuthenticationPacket;
import io.shardingcat.backend.postgresql.packet.AuthenticationPacket.AuthType;
import io.shardingcat.backend.postgresql.packet.BackendKeyData;
import io.shardingcat.backend.postgresql.packet.CommandComplete;
import io.shardingcat.backend.postgresql.packet.CopyInResponse;
import io.shardingcat.backend.postgresql.packet.CopyOutResponse;
import io.shardingcat.backend.postgresql.packet.DataRow;
import io.shardingcat.backend.postgresql.packet.EmptyQueryResponse;
import io.shardingcat.backend.postgresql.packet.ErrorResponse;
import io.shardingcat.backend.postgresql.packet.NoticeResponse;
import io.shardingcat.backend.postgresql.packet.NotificationResponse;
import io.shardingcat.backend.postgresql.packet.ParameterStatus;
import io.shardingcat.backend.postgresql.packet.PasswordMessage;
import io.shardingcat.backend.postgresql.packet.PostgreSQLPacket;
import io.shardingcat.backend.postgresql.packet.ReadyForQuery;
import io.shardingcat.backend.postgresql.packet.ReadyForQuery.TransactionState;
import io.shardingcat.backend.postgresql.packet.RowDescription;
import io.shardingcat.backend.postgresql.utils.PacketUtils;
import io.shardingcat.backend.postgresql.utils.PgPacketApaterUtils;
...
public class PostgreSQLBackendConnectionHandler extends BackendAsyncHandler {
	...
	private void doProcessBusinessQuery(PostgreSQLBackendConnection con,
		...
		BufferArray bufferArray = ShardingCatServer.getInstance().getBufferPool()
				.allocateArray();
		...
		while (itor.hasNext()) {
			bufferArray = ShardingCatServer.getInstance().getBufferPool()
					.allocateArray();
		...
		bufferArray = ShardingCatServer.getInstance().getBufferPool().allocateArray();
	 ...
	 * 
	 * 
	 * @see io.shardingcat.net.handler.BackendAsyncHandler#handleData(byte[])
	 */
	@Override
