...
import io.shardingcat.memory.unsafe.utils.sort.PrefixComparator;
import io.shardingcat.memory.unsafe.utils.sort.PrefixComparators;
import io.shardingcat.memory.unsafe.utils.sort.RowPrefixComputer;
import io.shardingcat.memory.unsafe.utils.sort.UnsafeExternalRowSorter;
...
public class DataNodeMergeManager extends AbstractDataNodeMerge {
     ...
     * sorter需要的上下文环境
     */
    private final ShardingCatMemory shardingCatMemory;
    ...
    public DataNodeMergeManager(MultiNodeQueryHandler handler, RouteResultset rrs,AtomicBoolean isMiddleResultDone) {
        ...
        this.shardingCatMemory = ShardingCatServer.getInstance().getShardingCatMemory();
        this.memoryManager = shardingCatMemory.getResultMergeMemoryManager();
        this.conf = shardingCatMemory.getConf();
    ...
    public void onRowMetaData(Map<String, ColMeta> columToIndx, int fieldCount) throws IOException {
	    ...
	    /**
             * shardingcat 中将 sql： select avg(xxx) from t
...
//            }
            ...
            globalSorter = new UnsafeExternalRowSorter(
                    ...
                    conf.getSizeAsBytes("shardingcat.buffer.pageSize","32k"),
                    false/**是否使用基数排序*/,
                    true/**排序*/);
        }


        if(conf.getBoolean("shardingcat.stream.output.result",false)
                && globalSorter == null
        ...
        try {
            for (; ; ) {
                ...
                if (pack == END_FLAG_PACK) {
                     ...
                     * 分片数据全部接收完成，进而将结果集全部发给你ShardingCat 客户端
                     */
