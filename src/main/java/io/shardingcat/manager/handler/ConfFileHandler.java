...
/**
 * ShardingCat conf file related Handler
 * 
 * @author wuzh
 */
public final class ConfFileHandler {
	...
	private static void checkXMLFile(String xmlFileName, byte[] data)
		...
		if (xmlFileName.equals("schema.xml")) {
			dtdStream = ShardingCatServer.class.getResourceAsStream("/schema.dtd");
			if (dtdStream == null) {
			...
			dtdStream = ShardingCatServer.class.getResourceAsStream("/server.dtd");
			if (dtdStream == null) {
				dtdStream = new ByteArrayInputStream(readFileByBytes(new File(
						confDir, "server.dtd")));
			}
		} else if (xmlFileName.equals("rule.xml")) {
			dtdStream = ShardingCatServer.class.getResourceAsStream("/rule.dtd");
			if (dtdStream == null) {
				dtdStream = new ByteArrayInputStream(readFileByBytes(new File(
						confDir, "rule.dtd")));
			}
