...
public class ShowDirectMemory {
    ...
    public static void execute(ManagerConnection c, int showtype) {
        useOffHeapForMerge = ShardingCatServer.getInstance().getConfig().
                getSystem().getUseOffHeapForMerge();
        processorBufferPoolType = ShardingCatServer.getInstance().getConfig().
                getSystem().getProcessorBufferPoolType();
        bufferPool = ShardingCatServer.getInstance().getBufferPool();
    ...
    public static void showDirectMemoryDetail(ManagerConnection c) {
        ...
        try {
            ...
            if (useOffHeapForMerge == 1) {
                ConcurrentHashMap<Long, Long> concurrentHashMap = ShardingCatServer.getInstance().
                        getShardingCatMemory().
    ...
    public static void showDirectMemoryTotal(ManagerConnection c) {
        ...
        try {
            ...
            if (useOffHeapForMerge == 1) {
                 ...
                 * ShardingCat能使用的DirectMemory是MaxDirectMemorySize*DIRECT_SAFETY_FRACTION大小，
                 * DIRECT_SAFETY_FRACTION为安全系数，为OS，Heap预留空间，避免因大结果集造成系统物理内存被耗尽！
                 */
                totalAvailable = (long) (Platform.getMaxDirectMemory() * ShardingCatMemory.DIRECT_SAFETY_FRACTION);
            } else {
            ...
            if (useOffHeapForMerge == 1) {
                 ...
                 */
                row.add(("" + ShardingCatMemory.DIRECT_SAFETY_FRACTION)
                        .getBytes(c.getCharset()));
            } else {
                row.add(("1.0")
                        .getBytes(c.getCharset()));
            ...
            if (useOffHeapForMerge == 1) {
                 ...
                 * 预留OS系统部分内存！！！
                 */
                resevedForOs = (long) ((1 - ShardingCatMemory.DIRECT_SAFETY_FRACTION) *
                        (Platform.getMaxDirectMemory() -
                                2 * ShardingCatServer.getInstance().getTotalNetWorkBufferSize()));
            }
