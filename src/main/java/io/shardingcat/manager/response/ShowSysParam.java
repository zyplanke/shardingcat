...
public class ShowSysParam {
	...
	public static void execute(ManagerConnection c) {
        ...
        SystemConfig sysConfig = ShardingCatServer.getInstance().getConfig().getSystem();
	...
	private static final String[] PARAMNAMES = {
		...
		"ShardingCat_processorCheckPeriod",
		"ShardingCat_dataNodeIdleCheckPeriod",
		"ShardingCat_dataNodeHeartbeatPeriod",
		"ShardingCat_bindIp",
		"ShardingCat_serverPort",
		"ShardingCat_managerPort"};
	...
	private static final String[] PARAM_DESCRIPTION = {
		...
		"主要用于指定NIOProcessor上共享的businessExecutor固定线程池大小。shardingcat在需要处理一些异步逻辑的时候会把任务提交到这个线程池中。新版本中这个连接池的使用频率不是很大了，可以设置一个较小的值。",
		"指定使用ShardingCat全局序列的类型。",
		...
		"SQL执行超时的时间，ShardingCat会检查连接上最后一次执行SQL的时间，若超过这个时间则会直接关闭这连接。默认时间为300秒",
