...
import static io.shardingcat.route.function.PartitionByCRC32PreSlot.*;
...
public class MigrateUtils {
   ...
   public static List<Range> removeAndGetRemain(List<Range> oriRangeList, List<Range> rangeList) {
    ...
    public static String getDatabaseFromDataNode(String dn){
        return    ShardingCatServer.getInstance().getConfig().getDataNodes().get(dn).getDatabase();
    }
    public static String getDataHostFromDataNode(String dn){
        return    ShardingCatServer.getInstance().getConfig().getDataNodes().get(dn).getDbPool().getHostName();
    }
