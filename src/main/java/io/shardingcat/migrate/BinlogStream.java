...
import io.shardingcat.util.DateUtil;
...
public class BinlogStream {
    ...
    public void setMigrateTaskList(List<MigrateTask> migrateTaskList) {
        ...
        for (MigrateTask migrateTask : migrateTaskList) {
           ...
           int count=Double.valueOf( ShardingCatServer.getInstance().getConfig().getDataHosts().get(dataHostTo).getSource().getSize()*0.8).intValue();
    ...
    private final class DelegatingEventListener implements BinaryLogClient.EventListener {
        ...
        private void exeSql(MigrateTask task,String sql){
            ...
            PhysicalDBNode dn = ShardingCatServer.getInstance().getConfig().getDataNodes().get(task.getTo());
