...
public class MigrateMainRunner implements Runnable {
    ...
    @Override public void run() {
        ...
        for (MigrateTask migrateTask : migrateTaskList) {
            ShardingCatServer.getInstance().getBusinessExecutor().submit( new MigrateDumpRunner(migrateTask,downLatch,sucessTask)) ;
        }
        try {
        ...
        if(sucessTask.get()==migrateTaskList.size())
        {
           ...
           //开始增量数据迁移
            PhysicalDBPool dbPool= ShardingCatServer.getInstance().getConfig().getDataHosts().get(dataHost);
