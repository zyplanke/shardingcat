/*
 * Copyright (c) 2013, OpenCloudDB/ShardingCat and/or its affiliates. All rights reserved.
...
import io.shardingcat.backend.jdbc.JDBCDatasource;
import io.shardingcat.backend.mysql.nio.MySQLDataSource;
import io.shardingcat.backend.postgresql.PostgreSQLDataSource;
import io.shardingcat.config.loader.ConfigLoader;
import io.shardingcat.config.loader.SchemaLoader;
import io.shardingcat.config.loader.xml.XMLConfigLoader;
...
import io.shardingcat.route.sequence.handler.DistributedSequenceHandler;
import io.shardingcat.route.sequence.handler.IncrSequenceMySQLHandler;
import io.shardingcat.route.sequence.handler.IncrSequenceTimeHandler;
import io.shardingcat.route.sequence.handler.IncrSequenceZKHandler;
...
public class ConfigInitializer {
	...
	private ShardingCatCluster initCobarCluster(ConfigLoader configLoader) {
		return new ShardingCatCluster(configLoader.getClusterConfig());
	}
	...
	private Map<String, PhysicalDBPool> initDataHosts(ConfigLoader configLoader) {
		...
		boolean isBooster="booster".equalsIgnoreCase(ZkConfig.getInstance().getValue(ZkParamCfg.SHARDINGCAT_SERVER_TYPE) ) ;
	...
	private PhysicalDBPool getPhysicalDBPool(DataHostConfig conf,
		...
		//连接数据库驱动，我们这里只讨论ShardingCat自己实现的native
