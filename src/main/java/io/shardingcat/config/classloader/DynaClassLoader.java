package io.shardingcat.config.classloader;
...
/**
 * used for shardingcat's catlet class loader ,catlet's class file is stored in
 * ShardingCat_home/catlet dir
 * 
 * @author wuzhih
 * 
 */
	...
	class MyDynaClassLoader extends ClassLoader {
		...
		public Class<?> loadClass(String name) throws ClassNotFoundException {
			if (name.startsWith("java") || name.startsWith("sun")
					|| name.startsWith("io.shardingcat")) {
		...
		private DynaClass searchFile(String classpath, String fileName) throws Exception {
			...
			if (f.isFile()) {
				...
				//classpath="D:\\code\\shardingcat\\ShardingCat-Server\\catlet\\";
