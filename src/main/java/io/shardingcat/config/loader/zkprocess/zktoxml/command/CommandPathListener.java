package io.shardingcat.config.loader.zkprocess.zktoxml.command;
...
import io.shardingcat.config.loader.zkprocess.zktoxml.ZktoXmlMain;
...
public class CommandPathListener implements PathChildrenCacheListener {
    ...
    public void reload(final String path) {
        ...
        lock.lock();
        try {
            ListenableFuture<Boolean> listenableFuture = ShardingCatServer.getInstance().getListeningExecutorService()
            ...
            }, ShardingCatServer.getInstance().getListeningExecutorService());
        } finally {
            lock.unlock();
        }
    }
