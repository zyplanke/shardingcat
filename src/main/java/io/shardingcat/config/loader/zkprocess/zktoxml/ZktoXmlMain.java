package io.shardingcat.config.loader.zkprocess.zktoxml;

import java.util.Set;

import io.shardingcat.config.loader.zkprocess.zktoxml.command.CommandPathListener;
import io.shardingcat.config.loader.zkprocess.zktoxml.listen.*;
...
import io.shardingcat.manager.handler.ZKHandler;
import io.shardingcat.migrate.MigrateTaskWatch;
    ...
    */
    public static void loadZktoFile() throws Exception {
        ...
        createTempNode(ZKUtils.getZKBasePath() + "line", ZkConfig.getInstance().getValue(ZkParamCfg.ZK_CFG_MYID),
                zkConn, ZkConfig.getInstance().getValue(ZkParamCfg.SHARDINGCAT_SERVER_TYPE));
