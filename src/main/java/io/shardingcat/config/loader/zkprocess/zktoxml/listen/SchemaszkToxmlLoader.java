...
import io.shardingcat.config.loader.console.ZookeeperPath;
import io.shardingcat.config.loader.zkprocess.comm.NotiflyService;
import io.shardingcat.config.loader.zkprocess.comm.ZookeeperProcessListen;
import io.shardingcat.config.loader.zkprocess.entity.Schemas;
import io.shardingcat.config.loader.zkprocess.entity.schema.datahost.DataHost;
import io.shardingcat.config.loader.zkprocess.entity.schema.datanode.DataNode;
import io.shardingcat.config.loader.zkprocess.entity.schema.schema.Schema;
import io.shardingcat.config.loader.zkprocess.parse.ParseJsonServiceInf;
import io.shardingcat.config.loader.zkprocess.parse.ParseXmlServiceInf;
import io.shardingcat.config.loader.zkprocess.parse.XmlProcessBase;
import io.shardingcat.config.loader.zkprocess.parse.entryparse.schema.json.DataHostJsonParse;
import io.shardingcat.config.loader.zkprocess.parse.entryparse.schema.json.DataNodeJsonParse;
import io.shardingcat.config.loader.zkprocess.parse.entryparse.schema.json.SchemaJsonParse;
import io.shardingcat.config.loader.zkprocess.parse.entryparse.schema.xml.SchemasParseXmlImpl;
import io.shardingcat.config.loader.zkprocess.zookeeper.DataInf;
import io.shardingcat.config.loader.zkprocess.zookeeper.DiretoryInf;
import io.shardingcat.config.loader.zkprocess.zookeeper.process.ZkDirectoryImpl;
import io.shardingcat.config.loader.zkprocess.zookeeper.process.ZkMultLoader;
...
public class SchemaszkToxmlLoader extends ZkMultLoader implements NotiflyService {
    ...
    */
    ...
    public boolean notiflyProcess() throws Exception {
        ...
        if(ShardingCatServer.getInstance().getStartup().get()) {
