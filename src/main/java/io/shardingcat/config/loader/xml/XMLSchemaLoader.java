...
import io.shardingcat.route.function.TableRuleAware;
...
import io.shardingcat.config.model.TableConfigMap;
...
public class XMLSchemaLoader implements SchemaLoader {
	...
	private void loadDataNodes(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; i++) {
			...
			String[] dnNames = io.shardingcat.util.SplitUtil.split(dnNamePre, ',', '$', '-');
			String[] databases = io.shardingcat.util.SplitUtil.split(databaseStr, ',', '$', '-');
			String[] hostStrings = io.shardingcat.util.SplitUtil.split(host, ',', '$', '-');
