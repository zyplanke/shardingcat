...
/**
 * 封装netty pooled Direct Memory 接口，为shardingcat提供内存分配功能
 * 由于ShardingCat目前使用ByteBuffer,而Netty分配的是ByteBuf，为了管理ByteBuf
 * 在ShardingCatMemoryAllocator中定义recycleMaps ByteBuffer(address) -->ByteBuf
...
public class NettyBufferPool implements BufferPool {
    ...
    ShardingCatMemoryAllocator allocator;
    private int chunkSize = 0;

    public NettyBufferPool(int chunkSize) {
        allocator = ShardingCatMemoryAllocator.getINSTANCE();
     ...
     * @return
     */
    public ShardingCatMemoryAllocator getAllocator() {
        return allocator;
    }
