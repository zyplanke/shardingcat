...
/**
 * Netty Direct Memory 分配器，为shardingcat提供内存池管理功能
 *
 * @author zagnix
 * @create 2017-01-18 11:01
 */

public class ShardingCatMemoryAllocator implements ByteBufAllocator {

    private static final Logger LOGGER = LoggerFactory.getLogger(ShardingCatMemoryAllocator.class);
    public final ConcurrentHashMap<Long,ByteBuf> recycleMaps = new ConcurrentHashMap<>();

    private final static ShardingCatMemoryAllocator INSTANCE =
           new ShardingCatMemoryAllocator(Runtime.getRuntime().availableProcessors()*2);
     ...
     */
    public ShardingCatMemoryAllocator(int numberOfArenas){
    ...
    public static ShardingCatMemoryAllocator getINSTANCE() {
        return INSTANCE;
    }
