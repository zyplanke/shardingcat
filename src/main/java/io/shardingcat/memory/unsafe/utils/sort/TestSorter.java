...
import io.shardingcat.memory.unsafe.memory.mm.DataNodeMemoryManager;
import io.shardingcat.memory.unsafe.memory.mm.MemoryManager;
import io.shardingcat.memory.unsafe.row.BufferHolder;
import io.shardingcat.memory.unsafe.row.StructType;
import io.shardingcat.memory.unsafe.row.UnsafeRow;
import io.shardingcat.memory.unsafe.row.UnsafeRowWriter;
import io.shardingcat.memory.unsafe.utils.ShardingCatPropertyConf;
import io.shardingcat.sqlengine.mpp.ColMeta;
import io.shardingcat.sqlengine.mpp.OrderCol;
...
public class TestSorter implements Runnable {
    ...
    public  void  runSorter( ShardingCatMemory shardingCatMemory,
                             MemoryManager memoryManager,
                             ShardingCatPropertyConf conf) throws NoSuchFieldException, IllegalAccessException, IOException {
        ...
        UnsafeExternalRowSorter sorter =
                new UnsafeExternalRowSorter(dataNodeMemoryManager,
                        ...
                        prefixComparator,
                        prefixComputer,
                        conf.getSizeAsBytes("shardingcat.buffer.pageSize","1m"),
    ...
    final  ShardingCatMemory shardingCatMemory ;
    final  MemoryManager memoryManager;
    final  ShardingCatPropertyConf conf;


        public TestSorter( ShardingCatMemory shardingCatMemory, MemoryManager memoryManager,ShardingCatPropertyConf conf) throws NoSuchFieldException, IllegalAccessException {
            this.shardingCatMemory = shardingCatMemory;
        ...
        public void run() {
            try {
                runSorter(shardingCatMemory,memoryManager,conf);
    ...
    public static void main(String[] args) throws Exception {
        ...
        ShardingCatMemory shardingCatMemory ;
        MemoryManager memoryManager;
        ShardingCatPropertyConf conf;

        shardingCatMemory = new ShardingCatMemory();
        memoryManager = shardingCatMemory.getResultMergeMemoryManager();
        conf = shardingCatMemory.getConf();

        for (int i = 0; i < TASK_SIZE; i++) {
            Thread thread = new Thread(new TestSorter(shardingCatMemory,memoryManager,conf));
            thread.start();
        }
