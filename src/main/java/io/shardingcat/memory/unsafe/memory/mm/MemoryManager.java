...
import io.shardingcat.memory.unsafe.memory.MemoryAllocator;
...
public abstract class MemoryManager {
  ...
  public MemoryManager(ShardingCatPropertyConf conf, int numCores, long onHeapExecutionMemory){
    this.conf = conf;
    this.numCores =numCores;
    maxOffHeapMemory = conf.getSizeAsBytes("shardingcat.memory.offHeap.size","128m");
...
public void releaseExecutionMemory(long numBytes, long taskAttemptId, MemoryMode memoryMode) {
  ...
  public final MemoryMode tungstenMemoryMode(){
    if (conf.getBoolean("shardingcat.memory.offHeap.enabled", false)) {
      assert (conf.getSizeAsBytes("shardingcat.memory.offHeap.size",0) > 0);
   ...
   *
   * If user didn't explicitly set "shardingcat.buffer.pageSize", we figure out the default value
  ...
  public long pageSizeBytes() {
    ...
    defaultSize = conf.getSizeAsBytes("shardingcat.buffer.pageSize", defaultSize);
