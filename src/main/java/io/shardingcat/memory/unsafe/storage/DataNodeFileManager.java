 ...
 *
 * Block files are hashed among the directories listed in shardingcat.local.dir
 */
public class DataNodeFileManager {
  ...
  public DataNodeFileManager(ShardingCatPropertyConf conf , boolean deleteFilesOnStop) throws IOException {

    this.conf = conf;
    this.deleteFilesOnStop = deleteFilesOnStop;


    subDirsPerLocalDir = conf.getInt("shardingcat.diskStore.subDirectories", 64);
   ...
   */
  private  List<File> createLocalDirs(ShardingCatPropertyConf conf) {

    String rootDirs = conf.getString("shardingcat.local.dirs","datanode");
