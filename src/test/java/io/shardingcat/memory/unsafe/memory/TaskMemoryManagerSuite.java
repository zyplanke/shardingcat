...
public class TaskMemoryManagerSuite {
  ...
  public void leakedPageMemoryIsDetected() {
    final DataNodeMemoryManager manager = new DataNodeMemoryManager(
      new ResultMergeMemoryManager(
        new ShardingCatPropertyConf().set("shardingcat.memory.offHeap.enabled", "false")
              .set("shardingcat.memory.offHeap.size","32768"),
              1,
        Long.MAX_VALUE
       ),
      0);
  ...
  public void encodePageNumberAndOffsetOffHeap() {
    final ShardingCatPropertyConf conf = new ShardingCatPropertyConf()
      .set("shardingcat.memory.offHeap.enabled", "true")
      .set("shardingcat.memory.offHeap.size", "1000");
  ...
  public void encodePageNumberAndOffsetOnHeap() {
    final DataNodeMemoryManager manager = new DataNodeMemoryManager(
      new TestMemoryManager(new ShardingCatPropertyConf().set("shardingcat.memory.offHeap.enabled", "false")), 0);
    final MemoryBlock dataPage = manager.allocatePage(256, null);
  ...
  public void cooperativeSpilling() throws InterruptedException {
    final TestMemoryManager memoryManager = new TestMemoryManager(new ShardingCatPropertyConf());
