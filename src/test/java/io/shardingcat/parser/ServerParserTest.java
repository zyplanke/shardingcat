...
public class ServerParserTest {
    ...
    @Test
    public void testShardingCatComment() {
        Assert.assertEquals(ServerParse.SELECT, 0xff & ServerParse.parse("/*#shardingcat:schema=DN1*/SELECT ..."));
        Assert.assertEquals(ServerParse.UPDATE, 0xff & ServerParse.parse("/*#shardingcat: schema = DN1 */ UPDATE ..."));
        Assert.assertEquals(ServerParse.DELETE, 0xff & ServerParse.parse("/*#shardingcat: sql = SELECT id FROM user */ DELETE ..."));
    }

    @Test
    public void testOldShardingCatComment() {
        Assert.assertEquals(ServerParse.SELECT, 0xff & ServerParse.parse("/*!shardingcat:schema=DN1*/SELECT ..."));
        Assert.assertEquals(ServerParse.UPDATE, 0xff & ServerParse.parse("/*!shardingcat: schema = DN1 */ UPDATE ..."));
        Assert.assertEquals(ServerParse.DELETE, 0xff & ServerParse.parse("/*!shardingcat: sql = SELECT id FROM user */ DELETE ..."));
    }
    ...
    @Test
    public void testShowShardingCatStatus() {
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_STATUS, ServerParseShow.parse("show shardingcat_status", 4));
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_STATUS, ServerParseShow.parse("show shardingcat_status ", 4));
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_STATUS, ServerParseShow.parse(" SHOW SHARDINGCAT_STATUS", " SHOW".length()));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse(" show shardingcat_statu", " SHOW".length()));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse(" show shardingcat_status2", " SHOW".length()));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse("Show shardingcat_status2 ", "SHOW".length()));
    }

    @Test
    public void testShowShardingCatCluster() {
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_CLUSTER, ServerParseShow.parse("show shardingcat_cluster", 4));
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_CLUSTER, ServerParseShow.parse("Show shardingcat_CLUSTER ", 4));
        Assert.assertEquals(ServerParseShow.SHARDINGCAT_CLUSTER, ServerParseShow.parse(" show  SHARDINGCAT_cluster", 5));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse(" show shardingcat_clust", 5));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse(" show shardingcat_cluster2", 5));
        Assert.assertEquals(ServerParseShow.OTHER, ServerParseShow.parse("Show shardingcat_cluster9 ", 4));
    }
