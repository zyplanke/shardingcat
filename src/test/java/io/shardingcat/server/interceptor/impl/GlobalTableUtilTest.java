...
public class GlobalTableUtilTest {
	...
	public void addColumnIfCreate() {
		...
		boolean contains = sql.contains("_shardingcat_op_time ");
		Assert.assertTrue(contains);
		sql = parseSql(originSql2);
		System.out.println(sql);
		Assert.assertFalse(sql.contains("_shardingcat_op_time int COMMENT '全局表保存修改时间戳的字段名'"));
	}
